...
testStatistic_with_covariates_Fit <- function(data, group.name, covariates , formula.str,
                                                      trend, robust) {
  ...
  return(list(d = d_values, s = s_values , corrected.logfc = corrected.logfc))
    ...
    pairwise_contrasts <- which(grepl(group.name, colnames(design.matrix)))
    
    pairwise_contrasts <- colnames(design.matrix)[pairwise_contrasts]
