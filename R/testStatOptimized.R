...
testStatOptimized <- function(isPaired, x) {
    ...
    if (length(sampleGroups) == 2) {
        ...
        if (!isPaired) {
            factorScaling <- sum(vapply(sampleGroups, ncol, numeric(1))) / prod(vapply(sampleGroups, ncol, numeric(1)))
            ...
            totalVariance <- rowSums(vapply(sampleGroups, function(group)
                rowSums((group - rowMeans(group, na.rm = TRUE))^2, na.rm = TRUE)), numeric(1))
