...
B <- 1000  # Number of bootstrap samples
K <- dim(Columbia.RealCase)[1]/2  # Set the value for K based on the data size
K <- floor(K) 
num_cores <- 30  # Number of cores for parallel processing
cluster <- makeCluster(num_cores)  # Create a parallel cluster
group.name <- "groups"
...
ggplot(limrots.result.df, aes(x = limrots.result$corrected.logfc, y = -log10(limrots.result$pvalue) )) +
