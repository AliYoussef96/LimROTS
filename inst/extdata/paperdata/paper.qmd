...
B <- 1000  # Number of bootstrap samples
K <- dim(soma)[1]/4  # Set the value for K based on the data size
K <- floor(K) 
num_cores <- 30  # Number of cores for parallel processing
cluster <- makeCluster(num_cores)  # Create a parallel cluster
...
limrots.result2 <- LimROTS(data.exp = soma, 
                          B = B, K = K, meta.info = meta.info, 
                          cluster = cluster, group.name = group.name, 
...
# Set metadata and formula for LimROTS analysis
meta.info <- names(soma@colData@listData)[1:4]
B <- 1000  # Number of bootstrap samples
K <- dim(soma)[1]/4  # Set the value for K based on the data size
K <- floor(K) 
num_cores <- 30  # Number of cores for parallel processing
cluster <- makeCluster(num_cores)  # Create a parallel cluster
group.name <- "group_comp"
formula.str <- "~ 0 + group_comp + gender + AGE + CNO"  # Formula for group comparison
# Run LimROTS analysis with trend and robust settings enabled
...
saveRDS(limrots.result, "../paperdata/Soma.noPCs.LimROTS.rds")
...
group1 <- sample(which(colData(soma)$group_comp == 1), length(which(colData(soma)$group_comp == 1))/1.8 )

group2 <- sample(which(colData(soma)$group_comp == 2), length(which(colData(soma)$group_comp == 2))/1.8 )

soma0.5 <- soma[, c(group1, group2) , drop = FALSE]

keep_samples <- colData(soma0.5)$CNO %in% names(which(table(colData(soma0.5)$CNO) <= 2))
soma0.5 <- soma0.5[, !keep_samples]

colData(soma0.5)$CNO <- droplevels(colData(soma0.5)$CNO)

soma0.5@assays@data@listData[["norm"]] <- preprocessCore::normalize.quantiles(as.matrix(soma0.5@assays@data@listData[["norm"]]))


# Set metadata and formula for LimROTS analysis
meta.info <- names(soma0.5@colData@listData)[1:3]
B <- 1000  # Number of bootstrap samples
K <- dim(soma0.5)[1]/4 # Set the value for K based on the data size
K <- floor(K) 
num_cores <- 30  # Number of cores for parallel processing
cluster <- makeCluster(num_cores)  # Create a parallel cluster
group.name <- "group_comp"
formula.str <- "~ 0 + group_comp + gender + AGE"  # Formula for group comparison
# Run LimROTS analysis with trend and robust settings enabled

trace(LimROTS, edit = T)

soma0.5.result <- LimROTS(data.exp = soma0.5, 
                          B = B, K = K, meta.info = meta.info, 
                          cluster = cluster, group.name = group.name, 
                          formula.str = formula.str, trend = TRUE, robust = TRUE, progress = TRUE, seed.cl = 1234)

saveRDS(soma0.5.result, "../paperdata/Soma0.5.noPCs.LimROTS.rds")


```

```{r}
soma0.5.result <- readRDS("Soma0.5.noPCs.LimROTS.rds")

plot(soma0.5.result$q_values)
hist(soma0.5.result$q_values)
hist(soma0.5.result$q_values$qvalues)
hist(soma0.5.result$FDR)

summary(soma0.5.result$q_values)
```

```{r}

soma.limrots <- readRDS("Soma.noPCs.LimROTS.rds")
soma0.5.limrots <- readRDS("Soma0.5.noPCs.LimROTS.rds")

soma.limrots <- row.names(soma.limrots$data)[which(soma.limrots$q_values$qvalues < 0.05)]
soma0.5.limrots <- row.names(soma0.5.limrots$data)[which(soma0.5.limrots$q_values$qvalues < 0.05)]

length(intersect(soma.limrots, soma0.5.limrots))
```
...
formula.str <- "~ 0 + group_comp + gender + AGE + CNO"
meta.info <- data.frame(colData(soma0.5), check.names = FALSE, row.names = row.names(colData(soma0.5)))
meta.info$sample.id <- NULL
...
fit2 <- contrasts.fit(fit, cont_matrix)
fit.ebayes <- eBayes(fit2, trend=TRUE, robust=TRUE)
limma.result.0.5 <- topTable(fit.ebayes, coef="group_comp1-group_comp2" , number = "Inf")

volcanoplot(fit.ebayes)
plotMD(fit.ebayes)

length(which(limma.result.0.5$adj.P.Val < 0.05))
...
keep_samples <- colData(soma)$CNO %in% names(which(table(colData(soma0.5)$soma) <= 2))
soma <- soma[, !keep_samples]
...
length(intersect(which(limma.result.0.5$adj.P.Val < 0.05) , which(limma.result$adj.P.Val < 0.05)))
```
