...
saveRDS(limrots.result, "D:/LimROTS/inst/extdata/paperdata/Soma.noPCs.LimROTS.rds")
...
soma0.5 <- soma[, sample(1:524, 262) , drop = FALSE]

keep_samples <- colData(soma0.5)$CNO %in% names(which(table(colData(soma0.5)$CNO) <= 2))
soma0.5 <- soma0.5[, !keep_samples]

# Set metadata and formula for LimROTS analysis
meta.info <- names(soma0.5@colData@listData)
B <- 1000  # Number of bootstrap samples
K <- dim(soma0.5)[1]/2  # Set the value for K based on the data size
K <- floor(K) 
num_cores <- 30  # Number of cores for parallel processing
cluster <- makeCluster(num_cores)  # Create a parallel cluster
group.name <- "group_comp"
...
soma0.5 <- LimROTS(data.exp = soma0.5, 
                          B = B, K = K, meta.info = meta.info, 
                          cluster = cluster, group.name = group.name, 
                          formula.str = formula.str, trend = TRUE, robust = TRUE, progress = TRUE, seed.cl = 1234)

saveRDS(soma0.5, "../paperdata/Soma0.5.noPCs.LimROTS.rds")


```

```{r}
soma0.5 <- readRDS("Soma0.5.noPCs.LimROTS.rds")

plot(soma0.5$q_values)
hist(soma0.5$q_values)
hist(soma0.5$q_values$qvalues)
hist(soma0.5$FDR)

summary(soma0.5$q_values)
```
...
length(intersect(soma.sig, soma0.5.sig))
```
