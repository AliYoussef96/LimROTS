...
# Load the dataset
load("../LimROTS paper/metabolomics/BM21.Case0.rda")
...
B <- 1000  # Number of bootstrap samples
K <- dim(BM21.Case0)[1]/2  # Set the value for K based on the data size
K <- floor(K) 
num_cores <- 30   # Number of cores for parallel processing
cluster <- makeCluster(num_cores)  # Create a parallel cluster
...
```{r}
length(which(limrots.result$q_values$qvalues < 0.05))

length(which(limrots.result$BH.pvalue < 0.05))

summary(limrots.result$q_values)
plot(limrots.result$q_values)
hist(limrots.result$q_values)

limrots.result.df <- data.frame(row.names  = row.names(limrots.result$data),
                                LimROTS.FC = limrots.result$corrected.logf,
                                FDR = limrots.result$FDR, p.value = limrots.result$pvalue, bh = limrots.result$BH.pvalue)
```
...
```{r}
length(which(rots.result$FDR < 0.05))
summary(rots.result$q_values)
plot(rots.result$q_values)
hist(rots.result$q_values)
hist(rots.result$p)
hist(rots.result$FDR)
```
...
saveRDS(limma.result, "../LimROTS paper/BM21.case0.limma.rds")
```
...
# Load the dataset
load("../LimROTS paper/metabolomics/BM21.Case1.rda")
...
meta.info <- "mixing.ratio"
B <- 1000  # Number of bootstrap samples
K <- dim(BM21.Case1)[1]/2  # Set the value for K based on the data size
K <- floor(K) 
num_cores <- 30   # Number of cores for parallel processing
cluster <- makeCluster(num_cores)  # Create a parallel cluster
group.name <- "mixing.ratio"
...
length(which(limrots.result$FDR < 0.05))
...
library(qvalue)
q <- qvalue(p.value, pi0.method = "bootstrap", lambda  = seq(0.01, 0.95, 0.01))
plot(q)
hist(q)
summary(q)

pi0est(p.value, pi0.method = "bootstrap")
...
length(which(limrots.result.df$q.value < 0.05))
length(which(limrots.result.df$fdr < 0.05))

limrots.result.df <- limrots.result.df[limrots.result.df$q.value < 0.05,]

length(which(grepl("_pluseffect", row.names(limrots.result.df)) ))

length(which(grepl("synthetic", row.names(limrots.result.df)) ))
```
