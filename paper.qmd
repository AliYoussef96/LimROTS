...
predictions_limrots <- limrots.result$q_values$qvalues < 0.01
...
ancova.result$FDR <- p.adjust(ancova.result$p.value, method = "fdr")
ancova.result <- saveRDS("../LimROTS paper/UPS1.Case2.ancova.rds")
...
trace(LimROTS::LimROTS , edit = T)

trace(LimROTS::calculateFDR , edit = T)
...
```{r}
length(which(limrots.result$FDR < 0.01))

length(which(limrots.result$BH.pvalue < 0.01))
...
```{r}
length(which(rots.result$FDR < 0.01))
