...
4 workers to execute LimROTS since it uses a parallel processing (using `BiocParallel` @BiocParallel)
...
we recommend to use at least 4 workers

On Windows OS, the user can define a `cluster` to work with more than 2 workers as follows:

```{r}
# cluster <- SnowParam(workers = 4) 
# Commented out in the vignette to ensure the limit set by the R_CHECK_LIMIT_CORES environment variable is not exceeded.
