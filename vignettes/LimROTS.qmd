...
# Set metadata and formula for LimROTS analysis
meta.info <- covariates.df
row.names(meta.info) <- meta.info$sample.id
...
cluster <- makeCluster(num_cores)  # Create a parallel cluster
group.name <- "groups"
formula.str <- "~ 0 + groups + tool"  # Formula for group comparison + injections

# Run LimROTS analysis with trend and robust settings enabled
limrots.result <- LimROTS(data.exp = both.norm, 
