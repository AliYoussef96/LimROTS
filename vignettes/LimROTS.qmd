...
limrots.result.df <- data.frame(proteins = row.names(limrots.result$data),
                                LimROTS.FC = limrots.result$corrected.logf,
                                FDR = limrots.result$FDR, bh = limrots.result$BH.pvalue, p.value = limrots.result$pvalue)
...
Additionally, 100 E. coli proteins were randomly selected, and an effect size of 10 was added to each. The software model was then run using these fake batches. The expected outcome is that only the UPS1 human proteins will be identified as truly significant, while none of the remaining proteins should show significant differences between the concentration groups.\
\
