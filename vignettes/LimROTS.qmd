...
\
The LimROTS approach initially use the limma @limma implement to simulate the intensity data of proteins and metabolites. A linear model is subsequently fitted using the design matrix. Subsequently, empirical Bayes variance shrinking is implemented. The adjusted standard error (SEpost = √(s2.post) \* unscaled SD) for each feature is computed, with the regression coefficient for each feature (indicating the impact of variations in the experimental settings) to get the moderated t-statistics. The ROTS @rots approach establishes optimality based on the largest overlap of top-ranked genes inside group-preserving bootstrap datasets. Refer to @elo2008reproducibility for further information on the reproducibility-optimization approach.
